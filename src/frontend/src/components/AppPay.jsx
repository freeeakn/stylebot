...
function AppPay({cart, user}) {

    const payHandle = async () => {
        let resp = await axios.post('api/v1/PayCart', {
            ...user,
            cart: cart,
        })
        console.log(resp);
    ...
    return (
        <>
            <button
            onClick={payHandle}
            className="flex items-center justify-center bg-gradient-to-br from-[#7bd989] via-[#2bbec8] p-2 rounded-xl shadow-xl hover:bg-lime-300 hover:drop-shadow-xl duration-200">
