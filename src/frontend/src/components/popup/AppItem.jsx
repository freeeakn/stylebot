...
function AppItem({ onShowItem, item, onAdd }) {
    const itemAnimation = {
        hidden: {
          scale: 0,
          opacity: 0,
        ...
        >
            ...
            >
                <div className="flex flex-row">
                    <h3 className=" bg-slate-100 py-2 p-4 rounded-xl shadow-xl">{item.title}</h3>
                </div>
                <div
                ...
                <img className="load rounded-xl shadow-2xl" src={item.img} alt="" />
                ...
                <p className=" bg-slate-100 py-2 p-4 rounded-xl shadow-xl text-sm">{item.desc}</p>
                <div className="flex flex-col gap-2 md:flex-row md:gap-0 justify-between ">
                    <p className="font-[600] bg-slate-100 p-2 rounded-xl shadow-xl">Price: {item.price}&#x24;</p>
                    <button onClick={() => onAdd(item)} className="flex items-center justify-center bg-slate-100 p-2 rounded-xl shadow-xl hover:bg-white hover:drop-shadow-xl duration-200">
